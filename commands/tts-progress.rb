...
cmd = "curl http://localhost:4000/status/progress/'#{tts_shared_secret}' > #{working_dir}/tts-progress.json"
Open3.popen2e(cmd) do |stdin, stdout_err, wait_thr|
  while line = stdout_err.gets
    puts "#{line}"
  end
...
file = File.open("#{working_dir}/tts-progress.json", 'r')
data = JSON.load file
file.close

cmd = "rm #{working_dir}/tts-progress.json"
Open3.popen2e(cmd) do |stdin, stdout_err, wait_thr|
  while line = stdout_err.gets
    puts "#{line}"
  end
